...
それどころか、プレビュー期間中に通常パスを通すことになる `google-cloud-sdk/bin` に置かれたバイナリ名が `spannercli` から `spanner-cli` へと変更されることで OSS spanner-cli と実行コマンドの名前が衝突する事故が発生しています。
これは Google の想定外の可能性が高いですが、 `PATH` 環境変数の設定状況によっては両方の併用を考えているユーザにとっては不便な変更です。
